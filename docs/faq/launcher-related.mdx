...
This is actually where Minecraft: Java Edition is ~~installed~~ downloaded.\
If you installed through the Windows installer, the launcher is located in `%AppData\sklauncher`\
If you downloaded the .jar or the portable .exe, it's in:\
...
The actual launcher isn't open-soruce at the moment, but there are plans for it to be in the future. However the code code isn't obfuscated, so you could decompile it and verify everything for yourself.\
The Windows installer, however, is open-source. You can find the repository [here.](https://github.com/sklauncher/installer)
...
3. Navigate to `%AppData\sklauncher` and run the `unins000.exe` fole located within that directory.
